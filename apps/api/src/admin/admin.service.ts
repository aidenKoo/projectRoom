...
export class AdminService {
  ...
  async getUserDetail(uid: string) {
    ...
    const profilePrivate = await this.profilePrivateRepository.findOne({ where: { userId: user.id } });
    const preference = await this.preferenceRepository.findOne({ where: { userId: user.id } });

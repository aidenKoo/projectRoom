...
export class ProfilesPrivateService {
  ...
  async upsert(
    userId: number,
    createDto: CreateProfilePrivateDto,
  ): Promise<ProfilePrivate> {
    const existing = await this.profilePrivateRepository.findOne({
      where: { userId },
    });
  ...
  async findByUserId(userId: number): Promise<ProfilePrivate | null> {
    const profile = await this.profilePrivateRepository.findOne({
      where: { userId },
    });
    return profile;
  }
  ...
  async update(
    userId: number,
    updateDto: UpdateProfilePrivateDto,
  ): Promise<ProfilePrivate> {
    const existing = await this.profilePrivateRepository.findOne({
      where: { userId },
    });
